...
class Helpers
{
    ...
    public static function message_validation_return($atributte) {
        return 'El valor del atributo '.$atributte.' no existe como referencia de la DIAN';
    }
    public static function message_validator() {
        $messages = [
            'required' => 'El atributo :attribute es requerido',
            'numeric' => 'El atributo :attribute debe ser numerico',
            'file' => 'El :attribute debe ser un archivo',
            'razon_social.max' => 'El :attribute debe ser de maximo 255 caracteres',
            'razon_social.min' => 'El :attribute debe ser de minimo 3 caracteres',
            'nombre_registrado_rut.max' => 'El :attribute debe ser de maximo 255 caracteres',
            'nombre_registrado_rut.min' => 'El :attribute debe ser de minimo 3 caracteres',
            'codigo_postal.max' => 'El :attribute debe ser de máximo 10 numeros',
            'digito_verificacion.max' => 'El atributo :attribute debe ser de de 0 a 9',
            'consecutivo_hasta.min' => 'El atributo :attribute debe ser mayor que el consecutivo desde',
            'fecha_fin_resolucion.after'=> 'El atributo :attribute debe ser mayor a la fecha de resolución inicial',
            'unique' => 'El atributo :attribute ya existe',
            'responsable_contable_nombre.min' => 'El atributo :attribute debe de minimo 3 caracteres',
            'responsable_contable_nombre.max' => 'El atributo :attribute debe de maximo 255 caracteres',
            'responsable_contable_identificacion.min' => 'El atributo :attribute debe de minimo 4 numeros',
            'responsable_contable_identificacion.max' => 'El atributo :attribute debe de maximo 12 numeros',
        ];
        return $messages;
    }
    ...
    public function setearMoneda($valor, $signo = '$')
    {
        if (!$valor) {
            return $signo . ' ' . '0,00';
        } else {
            $valorTruncado = (bcadd((string) $valor, '0', 2));
            $valorFinal = $this->separarNumero($valorTruncado);
            return $signo . ' ' . $valorFinal;
        }
    }
    /** FUNCION PARA SEPARAR EL NUMERO */

    public function separarNumero($numero, $separatorMil = '.', $separadorDecimal = ',', $cantidadDecimales = 2)
    {
        if (!$numero) {
            return '00' . $separadorDecimal . '00';
        }
        ...
        //no excepcionar, si $numero es un valor entero sin decimales
        if (count($numeroSeparado) > 1) {
            $decimales =  substr($numeroSeparado[1], 0, $cantidadDecimales) ?? $decimales;
        }

        $enterosFinales = '';

        $numeroTemporal = $numeroSeparado[0];

        while (strlen($numeroTemporal) > 3) {

            $enterosSiguientes =  $this->cortarTresUltimosNumeros($numeroTemporal);

            $enterosFinales = ($enterosFinales == '') ? $enterosSiguientes : $enterosSiguientes . $separatorMil . $enterosFinales;
        }

        if (strlen($numeroTemporal) > 0) {
            $enterosFinales = (strlen($numeroSeparado[0]) > 3) ? $numeroTemporal . $separatorMil . $enterosFinales : $numeroTemporal;
        }

        return $enterosFinales . $separadorDecimal . $decimales;
    }
    public function cortarTresUltimosNumeros(&$numeroTemporal)
    {

        $rest = substr($numeroTemporal, -3);

        $cantidadRestante = strlen($numeroTemporal) - 3;

        $numeroTemporal = substr($numeroTemporal, 0, $cantidadRestante);
